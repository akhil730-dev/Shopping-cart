   ...
   const {id} = useParams()
    ...
    useEffect(()=>{
        if(products.length > 0){
            ...
            productsCopy = productsCopy.filter((item)=>product.category === item.
        category)
            ...
            <div className="flex flex-col md:flex-row gap-16 mt-4">
                    ...
                    <div className="flex flex-col gap-3">
                        {product.image.map((image, index) => (
                            <div key={index} onClick={() => setThumbnail(image)} className="border max-w-24 border-gray-500/30 rounded overflow-hidden  cursor-pointer" >
                                <img src={image} className="w-full h-20 object-contain" alt={`Thumbnail ${index + 1}` } />
                            </div>
                        ))}
                    </div>

                    <div className="border border-gray-500/30  h-96 max-w-100 rounded overflow-hidden">
                        <img src={thumbnail} alt="Selected product" className="w-full  h-96 object-contain" />
                    </div>
                </div>
                ...
                {/*```````````````````` related products`````````````````  */ }     
                        ...
                        <div className="flex flex-col items-center mt-20">
                            ...
                            <button onClick={()=>{navigate('/products');scrollTo(0,0)}} className="mx-auto cursor-pointor px-12 my-6 py-2.5 border rounded text-primary hover:bg-primary/10 transition ">See more</button>
                        </div>
        </div>
    );
};
